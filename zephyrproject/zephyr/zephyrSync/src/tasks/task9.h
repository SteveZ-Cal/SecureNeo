...
#define TASK9_NUM_ITERATION 5

// static const struct device *gpio_ct_dev_task3 = DEVICE_DT_GET(DT_NODELABEL(gpio_lo));

// uint32_t task3_to_wait = 1000000; 

void neorv32_gpio_raw9(struct device *gpio_ct_dev_task, uint32_t gpio_pin, uint32_t task_to_wait, uint32_t num_iteration){

    int ret;

	/** ADD PC FETCH **/
	uint32_t PC_fetch = NEORV32_CFS->REG[NEORV32_PC_REG];
	printf("PC Fetch at NEORV32_GPIO_RAW9: [%x]\n", PC_fetch);
...
void neorv32_gpio_raw9_v2(struct device *gpio_ct_dev_task, gpio_port_pins_t mask, gpio_port_value_t value, uint32_t task_to_wait, uint32_t num_iteration){
...
void task9(void)
{
	...
	static const struct device *gpio_ct_dev_task9 = DEVICE_DT_GET(DT_NODELABEL(gpio_lo));
	...
	uint32_t task9_to_wait = 1000000; 

	printf("Task 9 ID: %u\n", k_current_get());

	printf("Task9 BEGIN!!\n");

	if (!device_is_ready(gpio_ct_dev_task9)){
		return;
	}

	printf("Task9 Device Ready!!\n");
	
	int ret;
	ret = gpio_pin_configure(gpio_ct_dev_task9, 3, GPIO_OUTPUT_ACTIVE); // set gpio_lo pin 3 as output

	printf("Task9 Configure Ready!!\n");
	...
	while(true){
		...
		neorv32_gpio_raw9_v2(gpio_ct_dev_task9, myPortPins, myPortValue, task9_to_wait, TASK9_NUM_ITERATION);

		printf("InLoop >>E<< Task9\n");		
