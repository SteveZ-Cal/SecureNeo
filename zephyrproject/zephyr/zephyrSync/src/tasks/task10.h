...
#define TASK10_NUM_ITERATION 5

// static const struct device *gpio_ct_dev_task3 = DEVICE_DT_GET(DT_NODELABEL(gpio_lo));

// uint32_t task3_to_wait = 1000000; 

void neorv32_gpio_raw10(struct device *gpio_ct_dev_task, uint32_t gpio_pin, uint32_t task_to_wait, uint32_t num_iteration){

    int ret;

	/** ADD PC FETCH **/
	uint32_t PC_fetch = NEORV32_CFS->REG[NEORV32_PC_REG];
	printf("PC Fetch at NEORV32_GPIO_RAW10: [%x]\n", PC_fetch);
    ...
    for (uint32_t i = 0; i < num_iteration; i++){
		...
		printf("PC Fetch at gpio_pin_set_raw10: [%x]\n", PC_fetch1);

		k_busy_wait(task_to_wait);

		/** ADD PC FETCH **/
		uint32_t PC_fetch2 = NEORV32_CFS->REG[NEORV32_PC_REG];
		printf("PC Fetch at k_busy_wait10: [%x]\n", PC_fetch2);
...
void neorv32_gpio_raw10_v2(struct device *gpio_ct_dev_task, gpio_port_pins_t mask, gpio_port_value_t value, uint32_t task_to_wait, uint32_t num_iteration){
...
void task10(void)
{
	...
	static const struct device *gpio_ct_dev_task10 = DEVICE_DT_GET(DT_NODELABEL(gpio_lo));
	...
	uint32_t task10_to_wait = 1000000; 

	printf("Task 10 ID: %u\n", k_current_get());

	printf("Task10 BEGIN!!\n");

	if (!device_is_ready(gpio_ct_dev_task10)){
		return;
	}

	printf("Task10 Device Ready!!\n");
	
	int ret;
	ret = gpio_pin_configure(gpio_ct_dev_task10, 3, GPIO_OUTPUT_ACTIVE); // set gpio_lo pin 3 as output

	printf("Task10 Configure Ready!!\n");
	...
	while(true){
		...
		neorv32_gpio_raw4_v2(gpio_ct_dev_task10, myPortPins, myPortValue, task10_to_wait, TASK10_NUM_ITERATION);

		printf("InLoop >>E<< Task10\n");		
