...
#define TASK2_NUM_ITERATION 10
// static const struct device *gpio_ct_dev_task2 = DEVICE_DT_GET(DT_NODELABEL(gpio_lo));

// uint32_t task2_to_wait = 500000; // appropriate time for blinky led if times / 2

// extern void task2(void);

void neorv32_gpio_raw2(struct device *gpio_ct_dev_task, uint32_t gpio_pin, uint32_t task_to_wait, uint32_t num_iteration){

    int ret;

	// /** ADD PC FETCH **/
	// uint32_t PC_fetch = NEORV32_CFS->REG[NEORV32_PC_REG];
	// printf("PC Fetch at NEORV32_GPIO_RAW2: [%x]\n", PC_fetch);
...
void neorv32_gpio_raw2_v2(struct device *gpio_ct_dev_task, gpio_port_pins_t mask, gpio_port_value_t value, uint32_t task_to_wait, uint32_t num_iteration){
...
void task2(void)
{
	...
	//gpio_port_value_t myPortValue = 682;
	...
	if (!device_is_ready(gpio_ct_dev_task2)){
		return;
	}

	printf("Task2 Device Ready!!\n");
	
	int ret;
	ret = gpio_pin_configure(gpio_ct_dev_task2, 2, GPIO_OUTPUT_ACTIVE); // set gpio_lo pin 2 as output

	printf("Task2 Configure Ready!!\n");
	...
	while(true){
		...
		// NEORV32_CFS->REG[0] = current_thread; // set task2 thread ID to its correponding cfs_reg
		...
		// ret = gpio_pin_set_raw(gpio_ct_dev_task2, 2, 1);
		// if (ret!=0){
		// 	return;
		// }

		// k_busy_wait(task2_to_wait);

		// ret = gpio_pin_set_raw(gpio_ct_dev_task2, 2, 0);
		// if (ret!=0){
		// 	return;
		// }
		...
		//neorv32_gpio_raw2(gpio_ct_dev_task2, 2, task2_to_wait, 2);

		neorv32_gpio_raw1_v2(gpio_ct_dev_task2, myPortPins, myPortValue, task2_to_wait, TASK2_NUM_ITERATION);

		//NEORV32_CFS->REG[0] = 3;

		printf("InLoop >>>E<<< Task2\n");
