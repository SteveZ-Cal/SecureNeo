...
#define TASK6_NUM_ITERATION 5

// static const struct device *gpio_ct_dev_task3 = DEVICE_DT_GET(DT_NODELABEL(gpio_lo));

// uint32_t task3_to_wait = 1000000; 

void neorv32_gpio_raw6(struct device *gpio_ct_dev_task, uint32_t gpio_pin, uint32_t task_to_wait, uint32_t num_iteration){

    int ret;

	/** ADD PC FETCH **/
	uint32_t PC_fetch = NEORV32_CFS->REG[NEORV32_PC_REG];
	printf("PC Fetch at NEORV32_GPIO_RAW6: [%x]\n", PC_fetch);
...
void neorv32_gpio_raw6_v2(struct device *gpio_ct_dev_task, gpio_port_pins_t mask, gpio_port_value_t value, uint32_t task_to_wait, uint32_t num_iteration){
...
void task6(void)
{
	...
	static const struct device *gpio_ct_dev_task6 = DEVICE_DT_GET(DT_NODELABEL(gpio_lo));
	...
	uint32_t task6_to_wait = 1000000; 

	printf("Task 6 ID: %u\n", k_current_get());

	printf("Task6 BEGIN!!\n");

	if (!device_is_ready(gpio_ct_dev_task6)){
		return;
	}

	printf("Task6 Device Ready!!\n");
	
	int ret;
	ret = gpio_pin_configure(gpio_ct_dev_task6, 3, GPIO_OUTPUT_ACTIVE); // set gpio_lo pin 3 as output

	printf("Task6 Configure Ready!!\n");
	...
	while(true){
		...
		neorv32_gpio_raw6_v2(gpio_ct_dev_task6, myPortPins, myPortValue, task6_to_wait, TASK6_NUM_ITERATION);

		printf("InLoop >>E<< Task6\n");		
