...
//   // for(int i=1; i<64; i++){
//   //   NEORV32_CFS->REG[i] = 273;
//   // }
//   // neorv32_cfs_register_print(0, 63); // print out the initial cfs registers for debugging purpose
...
//     // if (NEORV32_CFS->REG[TASK0_STATE_REG] == TASK0_STATE_REG_CONTEXT_SWITCH){
...
//         uint32_t max_num_blocks_cal = 3; // up from 1 to 10 blocks
...
//         neorv32_uart0_printf("NEORV32_CFS->REG[CFS_CTRL_REG]: [%x]\n", NEORV32_CFS->REG[CFS_CTRL_REG]);

//         neorv32_uart0_printf("\n-------------Start of Hybrid Attestation-------------\n\n");
...
//             // Hybrid_Attestation(this_num_blocks_cal, IV_uint32_list[trial_num], clock_prescaler, clock_prescaler_threshold, mem_address, verbose_hybridAttestation);
//             Hybrid_Attestation(this_num_blocks_cal, IV_uint32_list[trial_num], clock_prescaler, clock_prescaler_threshold, mem_address, 0);
...
//         // NEORV32_CFS->REG[CFS_CTRL_REG] = CFS_CTRL_REG_SET; // unset, so Zephyr won't set all cfs_reg to 0
...
//     // else if (NEORV32_CFS->REG[TASK0_STATE_REG] == TASK0_STATE_REG_INTERRUPT){
...
//         neorv32_uart0_printf("Ready to Secure Context Switch\n");
...
void task0(void){
  ...
  //if (NEORV32_CFS->REG[TASK0_STATE_REG] == TASK0_STATE_REG_INTERRUPT){
    ...
    //   //latencyStart = neorv32_cpu_get_cycle(); // start of evluating latency for initialization
    // }
    ...
    //   //latencyEnd = neorv32_cpu_get_cycle(); // start of evluating latency for initialization
    // }
      
    // if(latencyVerbose)
    //   neorv32_uart0_printf("@@@@@@@@@@@@@@@@@ Latency of Initialization @@@@@@@@@@@@@@@@@: [%u]\n", cpu_get_mycycle(latencyStart, latencyEnd));

    // if(latencyVerbose) 
    //   neorv32_uart0_printf("TASK0 (init): Latency Start [%u]\n", latencyStart);
    // if(latencyVerbose)
    //   neorv32_uart0_printf("TASK0 (init): Latency End [%u]\n", latencyEnd);
    ...
    // STATE = 1 ==> Interrupt from Zephyr to Update Policy Table
    if (NEORV32_CFS->REG[TASK0_STATE_REG] == TASK0_STATE_REG_INTERRUPT){
    //if (NEORV32_CFS->REG[TASK0_STATE_REG] == TASK0_STATE_REG_BOOTING){
        ...
        neorv32_uart0_printf("Ready to Update Policy Table\n");
